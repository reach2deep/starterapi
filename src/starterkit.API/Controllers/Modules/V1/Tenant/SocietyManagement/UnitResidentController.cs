...
namespace starterkit.API.Controllers.Modules.V1.Tenant.SocietyManagement
{
    ...
    public class UnitResidentController : BaseApiController
    {
        ...
        /// </summary>
        [HttpGet("{id}")]
        [ProducesResponseType(typeof(ApiResponse<UnitResident>), 200)]
        ...
        /// </summary>
        [HttpGet]
        [ProducesResponseType(typeof(ApiResponse<IEnumerable<UnitResident>>), 200)]
        ...
        [HttpPost]
        [ProducesResponseType(typeof(ApiResponse<UnitResident>), 200)]
        public async Task<IActionResult> Create([FromBody] UnitResident resident)
        {
            var result = await _residentService.CreateAsync(resident);
            return Ok(result);
        }
        ...
        [HttpPut("{id}")]
        [ProducesResponseType(typeof(ApiResponse<UnitResident>), 200)]
        public async Task<IActionResult> Update(Guid id, [FromBody] UnitResident resident)
        {
            if (id != resident.Id)
                return BadRequest("ID mismatch");

            var result = await _residentService.UpdateAsync(resident);
            return Ok(result);
        }
        ...
        [ProducesResponseType(typeof(ApiResponse<bool>), 200)]
        public async Task<IActionResult> HasActiveResidents(Guid unitId)
        {
            var result = await _residentService.HasActiveResidentsAsync(unitId);
            return Ok(result);
        }
